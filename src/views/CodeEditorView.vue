...
function onNodeDragEnd (e: PointerEvent) {
  if (draggingNodeId) {
      const nodePos: { x: number; y: number } = graphStore.getNode(draggingNodeId)?.position!
...
const maskStyle = computed(() => ({
  maskImage: `radial-graadient(
    circkle 120px at ${mouseWorldPosX.value}px ${mouseWorldPosY.value}px,
    black 0%, 
    transparent 100%
  )`,
  WebkitMaskImage: `radial-gradient(
    circle 120px at ${mouseWorldPosX.value}px ${mouseWorldPosY.value}px,
    black 0%,
    transparent 100%
  )`
}))
...
<template>
  ...
  <div
    ...
    <div
      ...
      :style="canvasStyle" id="maskingLayer">
      
      <div v-for="node in graphStore.graph.nodes" class="z-10 inset-0 absolute">
        <HexNode
        :key="node.id"
        :node="node"
        :hex-size=hexSize
